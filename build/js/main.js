  ...
  case 'http://localhost:3000/pressCentre.html':
    ...
    const getDelelteIdenticalNewsInArray = (arrayFirst, arrayTwo) => {
      ...
      filterButton.addEventListener('click', () => {
        ...
        // console.log();
        // console.log();
        getRemoveList(); // функция для проверки массива и удаления при не прохождения условий отбова новостей //

        const getDeletedNewsOfArray = () => {
          // объединение сортированных массивов //
          arrayOfNewsFiltered = arrayFirst.concat(arrayTwo); // Удаление одинаковых элементов массива //

          const arrayOfNewsFirstFiltered = arrayOfNewsFiltered.filter((item, index, self) => index === self.indexOf(item));
          console.log(arrayOfNewsFirstFiltered.filter((item, index, self) => index === self.indexOf(item)), 'Готовый массив фильтрованный'); // Главная сортировка //

          console.log(arrayOfNewsFirstFiltered.filter(item => {
            console.log(String(filterMonth.value) === String(item.month)); // console.log(typeof String(filterMonth.value));
          ...
          }), 'Готовый массив фильтрованный new'); //   console.log(filterMonth.value, 'Выбранный месяц');
