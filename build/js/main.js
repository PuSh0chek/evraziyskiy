...
let arrayOfNewsFiltered = []; // Массив контента для новостей //
...
switch (window.location.href) {
  case 'http://localhost:3000/index.html':
    ...
    getContent(arrayOfContentNews); // Анимация для рассылки новостей //
  ...
  case 'http://localhost:3000/pressCentre.html':
    ...
    const pressContainer = document.querySelector('.press__content'); // ФУНКЦИИ ДЛЯ РАБОТЫ С НОВОСТЯМИ //
    ...
    const getPopup = () => {
      ...
      Array.from(pressContainer.children).forEach(element => {
        element.addEventListener('click', () => {
          ...
          createPopupForNews(); // Загрузка контента из требуемого объекта в popup //
